...
public class AttackNode : ActionNode
{
    ...
    private void Attack()
    {
        ...
        {
            cols = Physics2D.OverlapBoxAll((Vector2)treeComponent.transform.position - attackPos * movement.faceDirection, boxSize, 0);
        }
        else if (shape == AttackShape.Circle)
        {
            cols = Physics2D.OverlapCircleAll((Vector2)treeComponent.transform.position - attackPos * movement.faceDirection, circleRadius);
        }
