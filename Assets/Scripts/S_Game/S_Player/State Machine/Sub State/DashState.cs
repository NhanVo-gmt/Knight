...
public class DashState : AbilityState
{
    ...
    {
        vfx.SpawnPooledPrefab(data.dashData.vfx, movement.transform.position, movement.faceDirection);
    }
    ...
    public bool CanDash()
    {
        return Time.time > lastActiveTime + data.dashData.dashRefillTime && dashRemaining > 0 && !movement.isDashing;
    }
